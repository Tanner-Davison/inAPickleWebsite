...
function generateCourtInfo(court) {
    ...
    courtInfo.setAttribute('data-tilt-glare', 'true');      // for TILT.JS
    courtInfo.setAttribute('data-tilt-perspective', '2000');// for TILT.JS
    courtInfo.setAttribute('data-tilt-scale', '1');         // for TILT.JS
    courtInfo.setAttribute('data-tilt-speed', '400');       // for TILT.JS
    courtInfo.setAttribute('data-tilt-max', '30');          // for TILT.JS
    
    courtInfo.innerHTML = `                                 
        <h1>${court.name}'s Court</h1>
        <div id='dimensions'>
        <div id='courtWithWidth'>
        <div style='display:flex, flex-direction:column;font-size:18px; transform: translateZ(800px);'>
        <u>Width</u>
    ...
    const notesBtn = courtInfo.querySelector('#notesBtn');
    const backBtn = courtInfo.querySelector('#backBtn');
    const courtFront = courtInfo.querySelector('.courtFront');
    const courtBack = courtInfo.querySelector('.courtBack');
  
    notesBtn.addEventListener('click', () => {
      courtFront.classList.add('flipped');
      courtBack.classList.add('flipped');
    });
  
    backBtn.addEventListener('click', () => {
      courtFront.classList.remove('flipped');
      courtBack.classList.remove('flipped');
    });
